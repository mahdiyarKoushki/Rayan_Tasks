...
function EditTaskForm({
  ...
  const [category, setCategory] = useState<CategoryType>(task.category || 'work');
  const [priority, setPriority] = useState<PriorityLevel>(task.priority || 'medium');
  const [timeSlot, setTimeSlot] = useState<DayTimeSlot>(task.timeSlot || 'afternoon');
  const [startTime, setStartTime] = useState(task.startTime || '16:00');
  const [endTime, setEndTime] = useState(task.endTime || '17:00');
  const [date, setDate] = useState<string>(
    task.date || formatJalaliShort(getCurrentJalaliDate(), false)
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-hidden">
      ...
      <motion.div
        ...
        className="relative w-full max-w-xl max-h-[90vh] sm:max-h-[85vh] rounded-3xl glass-panel shadow-2xl z-10 glass-edge border border-white/30 dark:border-white/10 flex flex-col overflow-hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl"
      >
        {/* Sticky Header */}
          ...
          <div className="flex items-center gap-2.5 text-violet-600 dark:text-violet-400">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                تنظیم اولویت، بازه روز، ساعت و تقویم شمسی
              </span>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 overscroll-contain custom-scrollbar">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-1.5">
              {(['morning', 'noon', 'afternoon', 'evening', 'night'] as DayTimeSlot[]).map(
                (slotKey) => {
          ...
          <div className="space-y-2 p-3 rounded-2xl bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/10">
            <div className="flex items-center justify-between">
              ...
              >
                {showDatePicker ? 'بستن تقویم ✕' : 'تغییر تاریخ 📅'}
              </button>
            </div>
          ...
          {/* Time Ranges (Start / End) via PersianTimePicker */}
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl bg-white/40 dark:bg-white/5 border border-white/20 dark:border-white/10">
            <div className="flex items-center gap-2">
              <CheckCircle2
                className={`w-5 h-5 ${completed ? 'text-emerald-500' : 'text-slate-400'}`}
              />
              <div>
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400">
                  {completed ? 'علامت خورده به عنوان تکمیل‌شده' : 'در دست اقدام و ناتمام'}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="توضیحات، لینک‌ها یا یادداشت‌های مربوط به این تسک..."
              className="w-full py-2.5 px-3 rounded-2xl bg-white/50 dark:bg-white/5 border border-white/40 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 text-xs"
            />
          </div>
        </div>

        {/* Sticky Footer */}
        <div className="px-5 py-3.5 border-t border-slate-200/50 dark:border-white/10 bg-white/70 dark:bg-slate-900/80 backdrop-blur-md shrink-0 flex items-center gap-2.5">
          <button
            type="button"
            onClick={() => handleSave()}
            className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-bold text-sm shadow-lg shadow-violet-600/30 transition-all cursor-pointer flex items-center justify-center gap-1.5"
          >
            <Check className="w-4 h-4" />
            <span>ذخیره تغییرات تسک</span>
          </button>

          <button
            type="button"
            onClick={handleDelete}
            className={`py-3 px-4 rounded-2xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
              confirmDelete
